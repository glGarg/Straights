...
HandControl::HandControl(GuiView *view, int playerIndex) : Gtk::Box(Gtk::ORIENTATION_VERTICAL), view_(view), cardBoxes_(13), cardButtons_(13), cardImages_(13), index_(playerIndex),
...
void HandControl::setHand(std::vector<std::string>& hand, DeckGui *deckGui)
{
	for (int i = 0; i < hand.size(); ++i)
    {
        ...
        cardButtons_[i]->signal_clicked().connect(sigc::bind<std::string, Gtk::Button *>(sigc::mem_fun(*this, &HandControl::playCard), hand[i], cardButtons_[i]));
    }
}
